...
const initialFamilies: Family[] = [
    { idFamilies: 1, name: "Columna", descriptions: "...", componentItemn: "...", image: "/assets/columna.png", subConcepto: [] },
];
...
export const dataHelper = {
    // LECTURA LOCAL
    getTechnicalFamilies(): Family[] { return getInitialData(STORAGE_KEYS.FAMILIES, initialFamilies); },
    getFamilyById(id: number) { return this.getTechnicalFamilies().find((f) => f.idFamilies === id); },
    getSubConceptById(fid: number, cid: number) { return this.getFamilyById(fid)?.subConcepto?.find((s) => s.conceptId === cid); },
    getFormativeConcepts(): Formative[] { return getInitialData(STORAGE_KEYS.FORMATIVES, initialFormatives); },
    getFormativeById(id: number) { return this.getFormativeConcepts().find((c) => c.conceptId === id); },
    ...
    // 1. Proponer Familia
    async addTechnicalFamily(payload: { name: string; descriptions?: string }): Promise<void> {
    ...
    async addFormativeConcept(payload: { name: string; description: string }): Promise<void> {
        const userId = getCurrentUserId();
        // Enviamos a /aportes
        await postToApi("aportes", {
            idUsuario: userId,
    ...
    async addSubConcept(familyId: number, payload: { name: string; description?: string }): Promise<void> {
        const userId = getCurrentUserId();
        // Enviamos a /aportes
        await postToApi("aportes", {
            idUsuario: userId,
    ...
    // --- REVISIÓN (ADMIN) ---
    async reviewAporte(idAporte: number, estado: "APROBADO" | "RECHAZADO", motivo: string = ""): Promise<void> {
